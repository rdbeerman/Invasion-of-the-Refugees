...
function as.harm(target)
  local vec3 = target:getPoint()
  if math.random(0, 10) <= as.harmWeaponChance * 10 then
    local controller = target:getController()
    controller:setOption(0, 4)
    if as.debug == true then
      trigger.action.outText("Disabled weapon system", 5, false)
...
function as.eventHandler(event)
  if (2 == event.id) then
    if event.target:getGroup():getCategory() == 3 then  
      ...
      elseif weaponType == "AGM_88" or weaponType == "LD-10" or weaponType == "AGM_122" then
        as.harm(event.target)
...
do -- Sets up event handlers
  trigger.action.outText("ACT antiShip Init started", 5 , false)
  
  mist.addEventHandler(as.eventHandler)
end
